...
class EFModel extends SlotsModel{
	public function getOcupado($id, $vacantes){
		$anotados = $this->anotados($id, "SEL_Entrevista_Final__c");
	...
	public function ocuparTurno($email, $turno, $year){
		...
		$sObject1->Id = $id;
		$sObject1->SEL_Entrevista_Final__c = $turno;
		$response = $this->mySforceConnection->update(array ($sObject1), 'Contact');
		
		
		if ($response[0]->success == 1) {
			$this->enviarConfirmacion($id, "00XE0000001EY1X");
			//Crea 3 entrevistas finales, una para cada evaluador.
			$this->crearObjeto("Entrevista_Final__c", $id);
			$this->crearObjeto("Entrevista_Final__c", $id);
			$this->crearObjeto("Entrevista_Final__c", $id);
			return true;
		}
